...
def preprocess(
    ...
    src_sentence: List[int] = list()
    trg_sentence: List[int] = list()

    for word in raw_src_sentence[:max_len]:
        src_sentence.append(
            UNK if not word in src_word2idx else src_word2idx[word])

    trg_sentence.append(SOS)
    for word in raw_trg_sentence[:max_len - 2]:
        trg_sentence.append(
            UNK if not word in trg_word2idx else trg_word2idx[word])
...
def bucketed_batch_indices(
    ...
    for idx, (src_len, trg_len) in enumerate(sentence_length):
        ...
        for bidx, bucket in enumerate(buckets):
            ...
            src_max = max(bucket, key=lambda it: it[0])[0]
            src_min = min(bucket, key=lambda it: it[0])[0]

            if (src_len > src_max) and (src_len - src_min > max_pad_len):
                continue

            if (src_len < src_min) and (src_max - src_len > max_pad_len):
                continue

            trg_max = max(bucket, key=lambda it: it[1])[1]
            trg_min = min(bucket, key=lambda it: it[1])[1]

            if (trg_len > trg_max) and (trg_len - trg_min > max_pad_len):
                continue

            if (trg_len < trg_min) and (trg_max - trg_len > max_pad_len):
                continue

            bucket.append((src_len, trg_len))
            batch_indices_list[bidx].append(idx)
            is_done = True
            break

        if not is_done:
            buckets.append([(src_len, trg_len)])
...
def collate_fn(
    ...
    batched_samples.sort(key=lambda it: len(it[0]), reverse=True)
    src_max = len(max(batched_samples, key=lambda it: len(it[0]))[0])
    trg_max = len(max(batched_samples, key=lambda it: len(it[1]))[1])

    for src, trg in batched_samples:
        src += [PAD] * (src_max - len(src))
        trg += [PAD] * (trg_max - len(trg))

    src_sentences: torch.Tensor = torch.LongTensor(
        [x[0] for x in batched_samples])
    trg_sentences: torch.Tensor = torch.LongTensor(
        [x[1] for x in batched_samples])
    # END YOUR CODE
...
def test_preprocess():
    ...
    # First test
    src_sentence, trg_sentence = preprocess(
        french[0], english[0], french.word2idx, english.word2idx, max_len=100)
    ...
    # Second test
    src_sentence, trg_sentence = preprocess(
        french[1], english[1], french.word2idx, english.word2idx, max_len=5)
    ...
    src_sentence, trg_sentence = preprocess(
        raw_src_sentence, raw_trg_sentence, french.word2idx, english.word2idx, max_len=12)
...
def test_buckting():
    ...
    sentence_length = [(random.randint(min_len, max_len), random.randint(
        min_len, max_len)) for _ in range(dataset_length)]
    ...
    for batch in batch_indices:
        src_length, trg_length = zip(
            *list(sentence_length[idx] for idx in batch))
...
def test_collate_fn():
    ...
    # the first test
    batched_samples = [([1, 2, 3, 4], [1]), ([1, 2, 3], [1, 2]),
                       ([1, 2], [1, 2, 3]), ([1], [1, 2, 3, 4])]
    ...
    # the second test
    batched_samples = [([1], [1, 2, 3, 4]), ([1, 2], [1, 2, 3]),
                       ([1, 2, 3], [1, 2]), ([1, 2, 3, 4], [1])]
    src_sentences, trg_sentences = collate_fn(batched_samples)
    assert (src_sentences == torch.Tensor([[1, 2, 3, 4], [1, 2, 3, 0], [1, 2, 0, 0], [1, 0, 0, 0]])).all() and \
           (trg_sentences == torch.Tensor([[1, 0, 0, 0], [1, 2, 0, 0], [1, 2, 3, 0], [1, 2, 3, 4]])).all(), \
