...
class DotAttention(AttentionBase):
    def forward(self,
        ...
        dec: torch.Tensor = decoder_hidden.unsqueeze(2).permute(0, 2, 1)
        enc: torch.Tensor = encoder_hidden.permute(0, 2, 1)
        energy: torch.Tensor = torch.matmul(dec, enc).squeeze(1)
        energy[encoder_mask] = float('-inf')
        distribution: torch.Tensor = F.softmax(energy, dim=1)
        attention: torch.Tensor = torch.matmul(
            distribution.unsqueeze(2).permute(0, 2, 1),
            encoder_hidden).squeeze(1)
        # END YOUR CODE
...
class ConcatAttention(AttentionBase):
    ...
    def forward(self,
        ...
        concated: torch.Tensor = torch.cat((encoder_hidden,
                                            decoder_hidden
                                            .unsqueeze(1)
                                            .expand(encoder_hidden.shape)),
                                           2)
        concated = torch.matmul(self.W_a, concated.permute(0, 2, 1))
        energy: torch.Tensor = torch.matmul(self.v_a.unsqueeze(1).permute(1, 0),
                                            torch.tanh(concated)).squeeze(1)
        energy[encoder_mask] = float('-inf')
        distribution: torch.Tensor = F.softmax(energy, dim=1)
        attention: torch.Tensor = torch.matmul(
            distribution.unsqueeze(2).permute(0, 2, 1),
...
def test_dot_attention():
    ...
    encoder_mask = torch.Tensor(
        [[False] * (sequence_length - i) + [True] * i for i in range(0, batch_size)]).to(torch.bool)
    encoder_hidden = torch.normal(
        0, 1, [batch_size, sequence_length, hidden_dim])
    encoder_hidden[encoder_mask] = 0.
    decoder_hidden = torch.normal(
        0, 1, [batch_size, hidden_dim], requires_grad=True)
    decoder_mask = torch.Tensor([False] * batch_size).to(torch.bool)

    attention_function: Attention = DotAttention()

    attention, distribution = attention_function(
        encoder_hidden, encoder_mask, decoder_hidden, decoder_mask)
...
def test_concat_attention():
    ...
    encoder_mask = torch.Tensor(
        [[False] * (sequence_length - i) + [True] * i for i in range(0, batch_size)]).to(torch.bool)
    encoder_hidden = torch.normal(
        0, 1, [batch_size, sequence_length, hidden_dim * 2])
    encoder_hidden[encoder_mask] = 0.
    decoder_hidden = torch.normal(
        0, 1, [batch_size, hidden_dim * 2], requires_grad=True)
    decoder_mask = torch.Tensor([False] * batch_size).to(torch.bool)
